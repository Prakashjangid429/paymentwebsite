...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const storedUser = localStorage.getItem("Spiral Pay_user")
    if (storedUser) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    if (email && password.length >= 6) {
      ...
      setUser(mockUser)
      localStorage.setItem("Spiral Pay_user", JSON.stringify(mockUser))
      setIsLoading(false)
      return true
    }
  ...
  const logout = () => {
    setUser(null)
    localStorage.removeItem("Spiral Pay_user")
  }
